<?php

namespace Mwteam\Blog\App\Http\Requests;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;
use Mwteam\Blog\App\Models\BlogCategory;

class BlogCategoryRequest extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array
     */
    public function rules()
    {
        $parents = [-1 => 0] + BlogCategory::whereNull('parent_id')->pluck('id')->all();
        return [
            'name' => 'required|between:5,190|unique:blog_categories,name,' . $this->blogCategory->id,
            'language' => 'required|in:fa,en,ar',
            'parent_id' => [
                'nullable',
                Rule::in($parents),
            ],
        ];
    }

    public function messages()
    {
        return [
            'name.required' => 'وارد کردن نام دسته اجباری است.',
            'name.between' => 'طول نام دسته باید بین 5 تا 190 کاراکتر باشد.',
            'name.unique' => 'دسته بندی وارد شده تکراری می باشد.',
            'language.required' => 'انتخاب زبان دسته اجباری است.',
            'language.in' => 'زبان انتخابی مجاز نمی باشد.',
            'parent_id.in' => 'دسته بندی انتخابی مجاز نمی باشد.',
        ];
    }
}
